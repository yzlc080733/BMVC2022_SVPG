...
TIME_STAMP = 'MNIST_WTArate_%d_rep%d' % (args.num_action, args.rep)
...
class Net_FC():
    def __init__(self, dim_input, dim_output):
        ...
        # -----Mask--------------
        self.mask_weight = torch.ones_like(self.weight)
        ...
        # -----Spike-------------
        self.spike_response_length = 30
        spike_response = torch.ones([1, self.spike_response_length], dtype=torch.float32) / self.spike_response_length
        self.full_spike_time = 150
        # -----PARAMETERS--------
    ...
    def calculate_batch(self, input_img, test_flag=False):
        ...
        if self.SPIKE_MODE:
            ...
            for spike_i in range(self.full_spike_time):
                ...
                spike_record[:, 0:self.dim_hid_act, spike_i:(spike_i+self.spike_response_length)] = sample_hid_act
                spike_record[:, self.dim_hid_act:self.dim_all, spike_i:(spike_i+self.spike_response_length)] = sample_state
                # -----update q----------
    ...
    def hid_act_sample(self, q_hid_act):
        v_hid_act = torch.zeros_like(q_hid_act)             # sample one-hot vectors according to q_hid_act
        input_batch_size = q_hid_act.shape[0]
        # -----hidden------------
        q_hid = q_hid_act[:, 0:self.dim_hid]
        q_hid_reshape = q_hid.reshape([input_batch_size, self.num_hidden, self.dim_hidden])
